...
select 
    ...
    fn_librarytoauthority(lph.policy_name) as authority,
    ...
    ch.accrued_fine as fine,
        case
            when ch.overdue = 0 then 'No'
            else 'Yes'
        end as overdue,
    ch.date_time_discharged as discharged,
    ldph.policy_name as discharge_library,
    fn_librarytoauthority(ldph.policy_name) as discharge_authority
from chargehist ch
...
join policy lph on lph.policy_number = ch.library and lph.policy_type = 'LIBR'
join policy ldph on ldph.policy_number = ch.discharge_library and ldph.policy_type = 'LIBR'
union all
select 
    u.id as user_id,
    i.id as item,
    ...
    null as discharged,
    null as discharge_library,
    null as discharge_authority
from charge c
